...
if __name__=='__main__':
    ...
    while not done:
        ...
        if i%20==0:
            ...
            points_4D= cv2.triangulatePoints(R_old,R_new,pold,pnew)

            # update grid using 4D points
            for j in range(len(points_4D[0])):
                if (int(points_4D[0,j]/points_4D[3,j])+400)<800 and (int(points_4D[1,j]/points_4D[3,j])+400)<800:
                    if (int(points_4D[0,j]/points_4D[3,j])+400)>0 and (int(points_4D[1,j]/points_4D[3,j])+400)>0:
                        if np.abs(points_4D[2,j]/points_4D[3,j])>17:
                            grid_mask[int(points_4D[0,j]/points_4D[3,j])+int(location_new[0])+400,int(points_4D[1,j]/points_4D[3,j])+int(location_new[1])+400]= 0 # grid_mask[int(points_4D[0,j]/points_4D[3,j])+int(location_new[0])+400,int(points_4D[1,j]/points_4D[3,j])+int(location_new[0])+400]-255/N
